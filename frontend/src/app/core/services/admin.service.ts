...
export class AdminService {
  ...
  addPlayer(player: Player): Observable<any> {
    return this.http.post(this.playersUrl, player).pipe(tap(() => this.loadPlayers()));
  }

  updatePlayer(player: any): Observable<any> {
    return this.http
      .put(`${this.playersUrl}/${player.id}`, player)
      .pipe(tap(() => this.loadPlayers()));
  }
  ...
  addTeam(team: any): Observable<any> {
    return this.http.post(this.teamsUrl, team).pipe(tap(() => this.loadTeams()));
  }

  updateTeam(team: any): Observable<any> {
    return this.http.put(`${this.teamsUrl}/${team.id}`, team).pipe(tap(() => this.loadTeams()));
  }
